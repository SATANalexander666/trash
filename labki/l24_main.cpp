...
void transformTree(link tree) 
{
	...
	if (tree) 
	{
		if (tree->data == '*') 
		{
			if ((isCH(tree->left->data)) && (tree->left->right == NULL) && (tree->left->left == NULL) & (tree->right->data == '+')) 
			{
				tree->data = '+';
			...
			else if ((isCH(tree->right->data) && (tree->right->right == NULL) && (tree->right->left && (tree->left->data == '+'))))
			{
				tree->data = '+';
